...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .strength-bar { flex: 1; height: 3px; border-radius: 2px; background: #e2e8f0; transition: all 0.3s; }
        ...
        .strength-text { font-size: 10px; margin-top: 4px; font-weight: 500; }
        ...
        .checkbox-wrapper { display: flex; align-items: flex-start; gap: 8px; cursor: pointer; margin-bottom: 18px; }
        .checkbox-wrapper input { width: 16px; height: 16px; accent-color: var(--primary); flex-shrink: 0; margin-top: 1px; }
        .checkbox-label { font-size: 11px; color: #64748b; line-height: 1.5; }
...
<body>
    ...
    <div class="auth-container">
        <div class="brand-side">
            <div class="brand-content">
                ...
                <h1 class="brand-title">Mulai Kelola Toko Online Anda</h1>
                <p class="brand-desc">Daftar gratis dan dapatkan akses ke semua fitur Warasa. Buat produk, analisis campaign, dan optimasi penjualan dalam satu platform.</p>
                <div class="brand-features">
                    <div class="brand-feature"><div class="brand-feature-icon"><i class="fas fa-infinity"></i></div><span>Gratis selamanya untuk akun dasar</span></div>
                    <div class="brand-feature"><div class="brand-feature-icon"><i class="fas fa-bolt"></i></div><span>Generate produk dengan AI cepat</span></div>
                    <div class="brand-feature"><div class="brand-feature-icon"><i class="fas fa-shield-halved"></i></div><span>Data aman dan terenkripsi</span></div>
                </div>
            </div>
        </div>
        <div class="form-side">
            <div class="form-wrapper">
                ...
                <div class="form-header animate-in">
                    ...
                    <p class="form-subtitle">Bergabung dengan Warasa sekarang juga</p>
                </div>
                <div class="auth-card animate-in">
                    <form method="POST" action="{{ route('register') }}" id="registerForm">
                        @csrf
                        <div class="form-group">
                            <label class="form-label"><i class="fas fa-user form-label-icon"></i>Nama Lengkap</label>
                            <input type="text" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus class="form-input @error('name') error @enderror" placeholder="John Doe">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required autocomplete="email" class="form-input @error('email') error @enderror" placeholder="you@example.com">
                        ...
                        <div class="form-group">
                            ...
                            <div class="password-wrapper">
                                <input type="password" name="password" required autocomplete="new-password" class="form-input @error('password') error @enderror" placeholder="Min. 8 karakter" id="registerPassword" oninput="checkPasswordStrength(this.value)">
                        ...
                        <div class="form-group">
                            <label class="form-label"><i class="fas fa-check-circle form-label-icon"></i>Konfirmasi Password</label>
                            <div class="password-wrapper">
                                <input type="password" name="password_confirmation" required autocomplete="new-password" class="form-input" placeholder="Masukkan ulang password" id="registerPasswordConfirm">
                                <button type="button" class="password-toggle" onclick="togglePassword('registerPasswordConfirm', this)" tabindex="-1"><i class="fas fa-eye"></i></button>
                            </div>
                        </div>
                        <label class="checkbox-wrapper">
                            <input type="checkbox" name="terms" required>
                            <span class="checkbox-label">Saya setuju dengan <a href="#">Syarat & Ketentuan</a> dan <a href="#">Kebijakan Privasi</a></span>
                        </label>
                        <button type="submit" class="btn-submit" id="registerSubmitBtn"><i class="fas fa-user-plus"></i> Daftar Gratis</button>
                    </form>
                    ...
                    <div class="auth-footer">
                        <p class="auth-footer-text">Sudah punya akun? <a href="{{ route('login') }}" class="auth-footer-link">Masuk</a></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function checkPasswordStrength(password) {
            const bars = [document.getElementById('str1'), document.getElementById('str2'), document.getElementById('str3')];
            const text = document.getElementById('strengthText');
            bars.forEach(bar => { bar.className = 'strength-bar'; });
            text.className = 'strength-text'; text.innerText = '';
            if (!password) return;
            let strength = 0;
            ...
            let level, label;
            if (strength <= 2) { level = 'weak'; label = 'Lemah'; }
            else if (strength <= 3) { level = 'medium'; label = 'Sedang'; }
            else { level = 'strong'; label = 'Kuat'; }
            const activeBars = strength <= 2 ? 1 : strength <= 3 ? 2 : 3;
            for (let i = 0; i < activeBars; i++) { bars[i].classList.add('active', level); }
            text.className = 'strength-text ' + level;
            text.innerText = 'Kekuatan password: ' + label;
        }
