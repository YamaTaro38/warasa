...
@section('page-title', 'Edit Documentation')
@section('breadcrumb', 'Management > Documentation > Edit guide')
        ...
        <form action="{{ route('admin.documentations.update', $documentation) }}" method="POST">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $documentation->title) }}" class="form-input" required>
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-input" required>
                        @foreach(['getting-started','features','tutorials','faq','general'] as $cat)
                            <option value="{{ $cat }}" {{ old('category', $documentation->category) == $cat ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $documentation->sort_order) }}" class="form-input" min="0">
                </div>
                <div class="form-group">
                    <label class="form-label">Status</label>
                    <div style="display:flex;align-items:center;gap:8px;padding-top:8px;">
                        <label class="toggle-switch">
                            <input type="checkbox" name="is_published" value="1" {{ $documentation->is_published ? 'checked' : '' }}>
                ...
                <div class="form-group" style="grid-column:1/-1;">
                    ...
                    <textarea name="content" id="editor" rows="15" class="form-input" style="display:none;">{{ old('content', $documentation->content) }}</textarea>
                    <div id="editor-container" style="border:1px solid var(--gray-300);border-radius:var(--radius-sm);min-height:400px;">{!! old('content', $documentation->content) !!}</div>
                    @error('content')
                        <div style="color:var(--danger);font-size:11px;margin-top:4px;">{{ $message }}</div>
                    @enderror
                </div>
